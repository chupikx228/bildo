...
from tests.generation.template_fixtures import BUILDERS, TemplateKey, build_template_document, select_template
...
def test_different_keywords_produce_different_documents() -> None:
    names = {build_template_document(prompt, None).name for prompt in TEMPLATE_PROMPTS.values()}

    assert len(names) == len(TEMPLATE_PROMPTS)


@pytest.mark.parametrize(("key", "prompt"), list(TEMPLATE_PROMPTS.items()))
def test_build_template_document_matches_selected_template(key: TemplateKey, prompt: str) -> None:
...
def test_build_template_document_is_valid_app_document(prompt: str) -> None:
    document = build_template_document(prompt, None)
...
@pytest.mark.parametrize("prompt", list(TEMPLATE_PROMPTS.values()))
def test_build_template_document_keeps_prompt_and_timestamps(prompt: str) -> None:
    document = build_template_document(prompt, None)
...
@pytest.mark.parametrize("prompt", list(TEMPLATE_PROMPTS.values()))
def test_build_template_document_has_screens_reachable_from_navigation(prompt: str) -> None:
    document = build_template_document(prompt, None)
...
@pytest.mark.parametrize("prompt", list(TEMPLATE_PROMPTS.values()))
def test_build_template_document_routes_are_bare_identifiers(prompt: str) -> None:
    document = build_template_document(prompt, None)
...
@pytest.mark.parametrize("prompt", list(TEMPLATE_PROMPTS.values()))
def test_build_template_document_navigate_actions_point_at_existing_screens(prompt: str) -> None:
    document = build_template_document(prompt, None)
...
@pytest.mark.parametrize("prompt", list(TEMPLATE_PROMPTS.values()))
def test_build_template_document_node_ids_are_unique(prompt: str) -> None:
    document = build_template_document(prompt, None)
...
def test_build_template_document_uses_given_name() -> None:
    document = build_template_document("трекер привычек", "Мои привычки")

    assert document.name == "Мои привычки"


def test_build_template_document_gives_unique_ids() -> None:
    first = build_template_document("трекер привычек", None)
    second = build_template_document("трекер привычек", None)
