...
│   │
│   ├── generation/              # ДОМЕН: промпт → AppDocument (LLM)
...
Для долгих операций фронту нужен статус. Сейчас во фронтовом контракте этого нет — при реализации согласуй: либо `POST` возвращает `task_id` и появляется `GET /api/tasks/{id}`, либо соединение держится через SSE. **Это меняет контракт — фиксируй в [`../api-contract.md`](../api-contract.md), а не только в коде.**
...
- **Генерация документа из промпта.** В прототипе — выбор одного из 5 готовых шаблонов по ключевым словам, поверх опционально дешёвый classify-запрос к OpenAI. Нужно решить: остаётся ли это отправной точкой или сразу делается настоящая LLM-генерация JSON (тогда обязательна валидация ответа моделью `AppDocument` и повтор при невалидном ответе).
