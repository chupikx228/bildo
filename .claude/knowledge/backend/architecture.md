...
## 9.3 Каталог моделей и выбор модели на запрос (BIL-42)

Модель для генерации выбирает пользователь на каждый `POST /api/apps` (формы запросов — в [`../api-contract.md`](../api-contract.md#выбор-модели-генерации-bil-42)). Отдельного списка разрешённых моделей нет: заказчик подтвердил, что разрешены все модели RouterAI, поэтому валидация «модель разрешена» вырождается в «модель есть в каталоге шлюза».
...
| Метод | Что делает |
|---|---|
| `async ensure_fresh()` | обновляет кэш, если он пуст или протух; на свежем кэше — no-op без сети |
| `is_valid(model_id)` | проверка по кэшу, без сети и без `await` |
| `list_models()` | текущий кэш как `list[ModelInfo]` (`id` / `name` / `pro`) |

Каталог берётся с `GET {routerai_base_url}/models` — публичного эндпоинта, **работающего без ключа** (проверено ещё в BIL-15). Ходит туда `httpx` напрямую, а не `openai`: это обычный REST-ответ, а не chat-completion, и гонять его через SDK незачем. Отсюда же новая рантайм-зависимость `httpx` в `pyproject.toml` и `src.generation.model_catalog` в контракте инверсии зависимостей рядом с `llm_client`.

**TTL кэша — час** (`CATALOG_TTL_SECONDS`). Каталог меняется, когда шлюз заводит новые модели, — это дни, а не минуты, так что час ничего практически не устаревает; при этом кэш обслуживает валидацию на каждом создании приложения, и ходить в сеть на каждый `POST /api/apps` было бы и медленно, и хрупко. Живёт кэш в памяти процесса: инстансов API может быть несколько, каждый греет свой — синхронизировать тут нечего, данные read-only и одинаковые.
...
**Ошибки различаются по тому, есть ли что отдавать.** Обновление упало, а в кэше уже что-то лежит — держим старое и пишем `WARNING`: устаревший список лучше отсутствующего. Кэш пуст и сеть не дала ничего — `ModelCatalogUnavailable` (502), а не пустой список: «моделей нет» и «каталог не доехал» — разные вещи, и фронту нужно уметь их различать.

Прогрев стоит в lifespan `src/main.py` рядом с созданием пула Arq, но **старт не роняет**: `ModelCatalogUnavailable` там ловится и логируется, приложение поднимается с пустым каталогом и наполнит его при первом обращении.

### Резолв модели

Живёт в `AppService.create_from_prompt` (`model: str | None`), не в Pydantic-схеме: проверке нужен каталог, а `field_validator` до коллабораторов сервиса не дотягивается.

1. `None`, пустая строка или `"auto"` → `settings.routerai_model`, каталог даже не трогается.
2. Иначе `await ensure_fresh()` и `is_valid` — не прошло, `InvalidModel` (422, текст по-русски) **до** `repository.create`, то есть ни записи, ни задачи в очереди.
...
`ModelInfo.pro` — **косметика**. Явного тарифа RouterAI не отдаёт, поэтому метка выводится эвристикой из `pricing.prompt` (цена за токен промпта, порог `1e-4` ≈ 100 ₽ за миллион токенов — примерно граница между «flash/mini» и «полноразмерными» моделями; на каталоге из 468 моделей помечается 146). Ни доступ, ни поведение генерации она не меняет и меняться не должна: появится у шлюза настоящее поле тарифа — эвристику заменить на него.
