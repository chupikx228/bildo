...
**Тот же порядок нарушен в `AppService.create_from_prompt`** (`enqueue` до коммита) — там окно закрывается тем, что воркер при `AppNotFound` валит задачу, а не пишет мусор, но чинить это надо тем же способом. Не забыть, когда дойдут руки.
