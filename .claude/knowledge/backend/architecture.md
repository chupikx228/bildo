...
Что это **не** закрывает: `chat_turn` по-прежнему на дефолтных 300 с, хотя ответ с предложенным документом — такая же полная генерация `AppDocument`. У хода чата нет состояния, которое зависает (провал виден через `error` в `/api/tasks/{id}`), но таймаут ударит по нему так же; это отдельная задача.
